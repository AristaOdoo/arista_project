    ...
    def adms_import(self, list_vals):
        ...
        for vals in list_vals:
            ...
            # Means do nothing
                ...
                pass
            # Special case for x_customer_vendor_group_branch_dependant, remove fal_business_type
        ...
        for key in vals:
            ...
            # Find the object based on field search
                ...
                # Except that
                    ...
                    if m2o_model.model == 'x_spk_payment':
                        m2o_business_type = m2o_model.field_id.sudo().filtered(lambda x: x.relation == 'fal.business.type' and x.ttype == 'many2one' and x.name == 'x_studio_fal_business_type')
                    # Special case for Partner. Because ADMS split it's partner to customer and vendor
                    # Both table have different approach
